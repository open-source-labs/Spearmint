...
const ExportFileModal = () => {
  ...
  const createImportStatements = () => {
    ...
    renderStatements.forEach((statement) => {
      setTestFileCode(testFileCode + `import ${ statement.componentName } from '${ statement.componentName }'`);
    })
  ...
  const saveTestFile = () => {
    ...
    fs.writeFile(
    path.join(__dirname, "../__tests__/beautifytest.js"), testFileCode, err => {
      if (err) throw err;
    });
  }
