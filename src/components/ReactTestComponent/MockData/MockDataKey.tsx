...
import { MockDataKeyProps } from '../../../utils/mockTypes';

const minusIcon = require('../../../assets/images/minus-box-outline.png');

const MockDataKey = ({ dispatchToMockData, mockDatumId, mockDatumKeyId, fieldKey, fieldType }: MockDataKeyProps) => {
  const handleChangeDelete = (e: React.MouseEvent) => {
    e.stopPropagation();
    dispatchToMockData(deleteMockDataKey(mockDatumId, mockDatumKeyId));
  };

  const handleChangeUpdateFieldKey = (e: React.BaseSyntheticEvent<KeyboardEvent>) => {
    e.stopPropagation();
    dispatchToMockData(updateMockDataKey(mockDatumId, mockDatumKeyId, e.target.value, fieldType));
  };

  const handleChangeUpdateFieldType = (e: React.BaseSyntheticEvent<KeyboardEvent>) => {
    e.stopPropagation();
